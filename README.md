...
**Collection status.** This release evaluates the verified two-snapshot analysis.
Daily re-observation of the same SKU--merchant panel and weekly discovery of new
merchant and product overlap are ongoing. Future releases will report survival and
...
minutes apart was byte-identical, so the observed month-apart differences are not
scanner nondeterminism. This is a two-snapshot catalog observation, not an intraday
survival estimate or a checkout-availability test (`OFFER-EPHEMERALITY-001`).

**Adaptive stopping has a detectable but small live-panel advantage.** On 152
held-out, single-currency episodes, the adaptive reservation-price rule reduced
failure-penalized purchase loss by 26.3 minor currency units relative to the strongest
fixed rule tuned on calibration data; its paired 95% bootstrap interval was
[-48.8, -8.6]. No replay exceeded its hard budget. This comparison uses
commit-or-continue semantics and treats episodes, rather than merchant or SKU
clusters, as independent bootstrap units (`STOPPING-ADVANTAGE-001`).

**The corpus leaves little room to separate policies.** The median held-out episode
contains two merchants and has a 1.001653 cross-merchant calibration-price ratio.
The generated [episode distribution table](artifacts/tables/episode_features.tex)
shows the full evaluated sample. Near-flat prices leave little scope for a price-based
stopping policy to differ, and two merchants do not distinguish the full dynamic
program from one-step lookahead. A calibrated simulation sweep extends beyond this
limited live-panel horizon and produces these evidence labels:
...
| Larger spread in selected cells | Adaptive stopping is favored, by up to 6.25% |

Each decision-table row reports its paired 95% bootstrap interval, held-out simulation
count, and replicate count. It is synthetic evidence calibrated to the corpus, not a
retail-outcome estimate or a universal shopping recommendation; the two fixed-rule
counterexamples do not establish one shared mechanism (`NO-ADVANTAGE-REGION-001`).
The table is regenerated by the build and reproduced in the paper; the site renders
the same values.
...
- Catalog observations are not checkout outcomes. Re-querying is technically possible,
	but it is a new observation; continuing does not reserve the earlier catalog offer.
	Prices in this corpus move slowly, consistent with the online price-rigidity
	literature.
