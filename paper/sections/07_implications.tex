...
\begin{enumerate}
  \item \textbf{Do merchants charge different prices for this product?} Measure the
    ...
    roughly \ResultNoAdvantageDispersionAtOrBelow{}$\times$, price variation is
    negligible, so a simpler fixed rule performs identically without dynamic
    reservation thresholds.
  \item \textbf{Can you afford to query available alternatives?} Check the decision
    grid in Table~\ref{tab:decision} against your query budget. When query capacity
    is very constrained relative to merchant count, inspect the effect size to
    confirm that your search horizon leaves enough room to act on what an inquiry
    reveals.
