...
is unavailable, with weight $q_t$, or a price $p$ drawn from a finite support with
weights $w_{t,p}$. Let $W_t = q_t + \sum_p w_{t,p}$.

The primary policy is commit-or-continue: an available catalog offer is accepted for
an immediate purchase attempt or the agent continues. Continuing does not reserve the
observed offer; a later query is treated as a new observation.
